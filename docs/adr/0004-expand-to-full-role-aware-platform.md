...
- **06 → 02 → 03** — 06 is the lowest-surface client feature (extends the existing `Stage`/dashboard spine); 02 defines the collaboration/request/next-action-owner model; 03's first-run experience then _consumes_ those requests and status rather than inventing a throwaway task list.
